//
//  MyReviewDetailKeywordView.swift
//  NanaLand
//
...
struct MyReviewDetailKeywordView: View {
    @EnvironmentObject var localizationManager: LocalizationManager
    @ObservedObject var viewModel: MyReviewDetailViewModel
    ...
    var body: some View {
        ...
        ZStack{
            NavigationView {
                VStack(spacing: 0) {
                    ...
                    VStack(alignment: .leading, spacing: 20) {
                        Text(.mood)
                            .font(.body_bold)
                        DetailTagCloudView(tags: Array(viewModel.keywordViewModel.keywords.prefix(5)), keywordViewModel: viewModel.keywordViewModel, localizationManager: _localizationManager)

                        Text(.companion)
                            .font(.body_bold)
                        DetailTagCloudView(tags: Array(viewModel.keywordViewModel.keywords[5..<11]), keywordViewModel: viewModel.keywordViewModel, localizationManager: _localizationManager)

                        Text(.amenities)
                            .font(.body_bold)
                        DetailTagCloudView(tags: Array(viewModel.keywordViewModel.keywords[11...]), keywordViewModel: viewModel.keywordViewModel, localizationManager: _localizationManager)
                    }
                    .padding()
            ...
            .toolbar(.hidden)
            .onAppear(){
                print(viewModel.selectedKeyword)
            }
        }
    }
}

struct DetailTagCloudView: View {
...
#Preview {
    MyReviewDetailKeywordView(viewModel: MyReviewDetailViewModel())
