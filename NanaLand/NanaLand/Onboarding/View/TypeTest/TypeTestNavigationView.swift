    ...
    var body: some View {
		NavigationStack(path: $typeTestVM.state.navigationPath) {
			TypeTestView(
				...
				titleSecondLine: "고를 때 \(nickname) 님은?",
			...
			.navigationDestination(for: TypeTestViewType.self) { viewType in
				...
				case .userTypeTest2:
					TypeTestView(
						...
						titleSecondLine: "세울 때 \(nickname) 님은?",
				...
				case .userTypeTest3:
					TypeTestView(
						...
						titleSecondLine: "정할 때 \(nickname) 님은?",
...
#Preview {
    TypeTestNavigationView(nickname: "123")
}
